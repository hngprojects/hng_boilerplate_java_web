...
public class VideoConversionServiceImpl implements VideoConversionService {
    ...
    @Override
    public VideoResponseDTO<VideoStatusDTO> startVideoProcess(MultipartFile video, String outputFormat) throws IOException {
        String jobId = VideoUtils.generateUuid();
        VideoSuite job = VideoUtils.videoSuite(jobId, VideoStatus.PENDING.toString(), null,
                JobType.CONVERT_VIDEO.toString(), VideoMessage.PENDING.toString(),
