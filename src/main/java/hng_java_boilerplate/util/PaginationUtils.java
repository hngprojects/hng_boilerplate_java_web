...
public class PaginationUtils {
    ...
    public static void validatePageNumber(int page, List<User> allUser) throws BadRequestException {
        int pageSize = 5;
        int totalUsers = allUser.size();
        int totalPages = (int) Math.ceil((double) totalUsers / pageSize);

        if (page < 1 || page > totalPages) {
            throw new BadRequestException("Invalid page number requested: " + page);
        }
    }
